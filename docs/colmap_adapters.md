...
| MappingInput v1/v2 | `MappingInput` composed from `CameraIntrinsics`, `FeatureSet`, and `CorrespondenceGraph` | `read_mapping_input`, `inspect_mapping_input`, `write_mapping_input` |
...
| SIFT text | `sceneio.FeatureSet` | `read_sift_features`, `write_sift_features` |
...
| Feature-match text | `sceneio.CorrespondenceGraph` | `read_feature_matches`, `write_feature_matches` |
| Sim3 text | `sceneio.Sim3` | `read_similarity_transform`, `write_similarity_transform` |
...
`sceneio.CameraModel` exposes the same names, ids, and ordered parameter
layouts, while CMake generates the compiled core lookup from that manifest.
Adapters return the same root `CameraIntrinsics`, `FeatureSet`,
`CorrespondenceGraph`, and `Sim3` types consumed by mapping and matching
contracts; no projection bridge or parallel record family is involved.
