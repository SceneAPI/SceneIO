...
| 🟡 USD / USDZ / historical USDC | `MeshScene` compatibility + `SceneGraph` | TinyUSDZ (Apache-2.0) | mesh R+W; rich static mesh/points/materials/Gaussians/cameras R+W | optional `sceneio[usd]`; U2 maps bounded stage hierarchy/metadata/static transforms and selected prims; C1 adds polygon meshes/points, C2 adds bounded PreviewSurface constants/textures and direct/subset bindings, C3 maps the official OpenUSD 26.08 float/half Gaussian particle schema with degree 0-3 SH and explicit conventions, and C4 maps static camera/render-product pairs to projection-equivalent `CameraRig` rows; historical USDC input is qualified only through crate 10 and later crates refuse before provider dispatch; C5-C7 close volume/instance/semantic payloads, the provider decision, and release evidence; OpenUSD remains a reference until the narrow TOST decision |
