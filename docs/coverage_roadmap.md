...
`a5e7fa4` has 50 readable, writable, inspectable, and streamed codecs; 28 have
bounded partial selectors. Local MSVC validation passes 2,919 tests with 4
documented skips. Normal CI run
...
collects the exact 2,923-test surface and passes its complete and focused
native jobs. Nonpublishing release run
...
The next execution order is: complete the repository-organization and
codec-backend performance gates, close default native sources for offline
builds, then finish animated WebP/APNG and RTMV.
...
- [x] The **compiler-instrumented native reliability lane** builds the core and
      ...
      The default-branch schedule retains the 100-case sweep. Both complete
      and focused jobs pass at `a5e7fa4`.
