...
- [ ] The **instrumented native reliability lane** builds the core and vendored
      libraries with ASan/UBSan/LSan, but the current remote job is not green:
      its helper removes an unconditionally imported pycolmap oracle during
      collection and its leak filter reports CPython/pydantic allocations.
      Close the test-environment/filtering defects before treating this gate as
      validated; scheduled runs still raise the mmap mutation sweep to 100
      cases.
