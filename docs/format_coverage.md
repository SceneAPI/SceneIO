...
| Offline native-source closure | 🟡 | lodepng, stb, tinyexr, SQLite, tinyobjloader, cgltf, miniz, and nlohmann/json are stored in-tree; zstd, fast_float, LAZperf, and libwebp still use pinned CMake `FetchContent` and must move under `src/cpp/third_party/` before the post-0.2 tier is called stable |
...
| In-tree native dependencies | 🟡 | permissive and license-indexed; four dependencies remain source-fetched as described by the offline-closure row |
