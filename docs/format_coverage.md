...
| `ColmapDatabase` | `match_graph` | ✅ record / ⬜ datatype | cameras, prior-focal flags, ordered `FeatureSet` values, `MatchGraph`, exact profile/application identity, schema user version, and current-profile recovered-camera reads |
...
| `colmap_db` | `ColmapDatabase` (`FeatureSet` + `MatchGraph`) | inspect exact profiles; R+W core subset, partial | stdlib **sqlite3** + **pycolmap 4.1.1** | exact structural identity for stock 3.13/4.1.1/current and owned MAXX; migration-derived pre-ownership databases remain legacy/unknown pending C1e import classification; current-upstream recovered `camera1`/`camera2` reads preserve endpoint-local NULL and prior flags; populated rig/frame/prior/MAXX companion rows remain guarded until C1c-C1d, and MAXX payload reads remain guarded until C1d; exact-profile writers remain C1e; exact pair ids and absent/empty core BLOB state; transactional legacy writer and selectors |
