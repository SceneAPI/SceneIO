...
| `MatchGraph` | `match_graph` | ✅ record / ⬜ datatype | canonical COLMAP image pairs and pair ids; ragged raw/verified u32 matches; optional scores, F/E/H, config, relative pose, and recovered endpoint cameras with independent presence/prior flags |
| `ColmapDatabase` | `match_graph` | ✅ record / ⬜ datatype | cameras, prior-focal flags, ordered `FeatureSet` values, `MatchGraph`, nested stock `ColmapRigFrameSet` and `ColmapPosePriorSet`, exact profile/application identity, schema user version, and current-profile recovered-camera reads |
...
| `ColmapPosePriorSet` | COLMAP DB companion | ✅ nested record | uint32 prior/sensor IDs plus uint64 correlated data IDs, image-linked 3.13 or generalized modern associations, int32 coordinate-system codes, SQL-presence flags, float64 position/gravity, and logical row-major 3x3 covariance |
...
| `colmap_db` | `ColmapDatabase` (`FeatureSet` + `MatchGraph` + nested companions) | inspect exact profiles; R+W core subset, partial | stdlib **sqlite3** + **pycolmap 4.1.1** | exact structural identity for stock 3.13/4.1.1/current and owned MAXX; migration-derived pre-ownership databases remain legacy/unknown pending C1e import classification; stock rig/frame and both stock prior layouts read losslessly with SQL presence and covariance wire order preserved; current-upstream recovered `camera1`/`camera2` reads preserve endpoint-local NULL and prior flags; MAXX payload reads remain guarded until C1d; exact-profile writers remain C1e; transactional legacy writer refuses new fields |
