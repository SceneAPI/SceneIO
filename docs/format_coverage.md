...
| `colmap_db` | `ColmapDatabase` (`FeatureSet` + `MatchGraph` + nested companions) | inspect exact profiles; R all represented stock/MAXX fields, W core subset, partial | stdlib **sqlite3** + **pycolmap 4.1.1** | exact structural identity for stock 3.13/4.1.1/current and owned MAXX; migration-derived pre-ownership databases remain legacy/unknown pending C1e import classification; stock rig/frame/prior and current recovered-camera reads are lossless; MAXX descriptors/colors/scores/provenance/quality/extended priors/markers/metadata-only video/ownership reads preserve SQL presence and wire bytes; exact-profile writes remain unavailable until C1e; transactional legacy writer refuses new fields |
