...
`nlohmann_json` 3.11.3, and `zstd` 1.5.6 are repository-contained, while
`fast_float`, `lazperf`, and `libwebp` are still obtained through CMake
`FetchContent`. Before the post-0.2 codec tier is called stable, vendor those
three exact audited revisions
...
3. **Repository source closure for the stable tier (R6).**
   ...
   3.0.2, nlohmann/json 3.11.3, and zstd 1.5.6 are repository-contained; the
   remaining closure set is fast_float 6.1.6, LAZperf 3.4.0 commit, and libwebp
