...
class BuiltinAssembly:
    ...
    def add_family(
        ...
        if actual_ids != expected_ids:
            raise ValueError(
                f"built-in family ids {actual_ids!r} do not match {expected_ids!r}"
            )
        unknown_ids = tuple(
            format_id
            for format_id in actual_ids
            if format_id not in self._canonical_ids
        )
        if unknown_ids:
            raise ValueError(f"unknown built-in codec ids: {unknown_ids!r}")
        collisions = tuple(
            format_id for format_id in actual_ids if format_id in self._definitions
        )
        if collisions:
    ...
    def finalize(self) -> tuple[Codec, ...]:
        ...
        missing_ids = tuple(
            format_id
            for format_id in self._canonical_ids
            if format_id not in self._definitions
        )
        extra_ids = tuple(
            format_id
            for format_id in actual_ids
        ...
        definitions = tuple(
            self._definitions[format_id] for format_id in self._canonical_ids
        )
...
def publish_builtin_definitions(
    ...
    if actual_ids != CANONICAL_BUILTIN_IDS:
        raise ValueError(
            f"built-in codec ids {actual_ids!r} do not match "
            f"{CANONICAL_BUILTIN_IDS!r}"
        )
    if registry:
