...
def _mesh_from_prim(prim):
    return _geometry.mesh_from_prim(
        prim,
...
def _validate_prim_shell(
    ...
    if prim.type_name == "Points":
        raise ValueError(
            "USD: UsdGeomPoints requires sceneio.read_scene(); "
            "the compatibility read() API returns MeshScene only"
        )
...
def read_usd(path: str | os.PathLike[str]):
    ...
    if any(
        ...
        for prim in _require_tinyusdz().traverse(stage)
    ):
        raise ValueError(
            "USD: UsdGeomPoints requires sceneio.read_scene(); "
            "the compatibility read() API returns MeshScene only"
        )
    return _stage_to_scene(stage)
