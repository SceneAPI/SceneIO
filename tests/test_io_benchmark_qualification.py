...
def _assembled_ids(specs, directory_specs):
    path_specs = [
        *runner.build_container_specs(0.001),
        *runner.build_media_path_specs(0.001),
    ]
    return (
...
def test_qualification_ledger_is_complete_immutable_and_checked():
    ...
    assert sum(item.mode == "timed" for item in ledger.values()) == 52
    assert (
        sum(
...
def test_assembled_sweep_is_exactly_the_repository_builtins():
    ...
    assert len(observed) == len(set(observed)) == 69
